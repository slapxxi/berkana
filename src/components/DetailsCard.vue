...
<template>
  <div class="grid gap-4 rounded-[40px] bg-white p-2">
    ...
    <h3 class="text-fluid-xl font-bold text-neutral-800">{{ props.title }}</h3>

    <div class="grid gap-1">
      <div
        v-for="[key, value] in Object.entries(props.details)"
        :key="key"
        class="flex flex-wrap items-center gap-2 text-neutral-600"
      >
        <div>{{ key }}:</div>
    ...
    <hr class="my-0 self-center text-neutral-200" />

    <div class="flex flex-wrap gap-2">
      <div class="flex items-center gap-1">
        <span class="translate-y-0.5 text-sm text-neutral-500">от</span>
        <span class="text-fluid-xl font-bold text-neutral-800">
          {{ props.price }}
        </span>
      </div>
